...
<section id="top_product" class="w-full bg-white flex flex-col lgx:flex-row md:items-center">
    ...
    <div class="w-full p-[25px] lgx:p-[100px] flex flex-col gap-10 text-light-black">
        <h2 class="text-[36px] l:text-[48px] text-brand">{{$data['top_product_title']}}</h2>
        <p class="font-semibold text-lg font-md">{{$data['top_product_short_description']}}</p>
        <div class="flex lgx:flex-wrap flex-col lgx:flex-row gap-6 lgx:items-center justify-between">
            @if($features_arr)
                @foreach($features_arr as $feature)
                    <div class="text-brand flex flex-row items-center justify-start gap-1">
                        <span>&check;</span>
        ...
        <a href="{{$data['top_product_link']}}" target="_blank"
           class="w-[170px] text-center bg-brand hover:bg-brand-hover text-white p-3 rounded">
            Перейти до товару
        </a>
    </div>
</section>
